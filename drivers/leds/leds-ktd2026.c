			...
			const struct i2c_device_id *id)
{
	...
	ret = i2c_smbus_write_byte_data(led->client , KTD_REG_LCFG, 0x00);//set current is 0.125mA
	if (ret < 0)
		goto err_exit;
	ret = i2c_smbus_write_byte_data(led->client , KTD_REG_LEDE, 0x00);//turn off leds
	if (ret < 0)
		goto err_exit;
	ret = i2c_smbus_write_byte_data(led->client , KTD_REG_RSTR, 0x08);//enter sleep mode
	if(ret < 0){
		goto err_exit;
	}
	else{
