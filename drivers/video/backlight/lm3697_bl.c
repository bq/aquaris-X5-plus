...
#define LM3697_REG_CURR_SINK_EN	0x19
#define LM3697_REG_BOOST		0x1A
...
#ifdef MDSS_BL_DEV
struct lm3697_bl_chip *g_chip;
int lm3697_mdss_backlight_set(u32 level)
{
	int bank_id = 0;
	int ret;
	u8 data;
	u8 reg_lsb[] = { LM3697_REG_BRT_A_LSB, LM3697_REG_BRT_B_LSB, };
	u8 reg_msb[] = { LM3697_REG_BRT_A_MSB, LM3697_REG_BRT_B_MSB, };
	struct lm3697_bl_chip *chip = g_chip;

	if (!chip)
		return -ENODEV;

	if (level > 0)
		ret = lm3697_bl_update_bits(chip, LM3697_REG_ENABLE, BIT(0), 1);
	else
		ret = lm3697_bl_update_bits(chip, LM3697_REG_ENABLE, BIT(0), 0);
	if (ret)
		return -EIO;

	if (level != 0)
		level = (level - 10) * 5 + 822;

	data = level & LM3697_BRT_LSB_MASK;
	ret = lm3697_bl_update_bits(chip, reg_lsb[bank_id],
		LM3697_BRT_LSB_MASK, data);
	if (ret)
		return -EIO;
	...
	ret = lm3697_bl_write_byte(chip, reg_msb[bank_id], data);
	if (ret)
		return -EIO;
...
static int lm3697_bl_configure(struct lm3697_bl *lm3697_bl)
{
	...
	lm3697_bl_write_byte(lm3697_bl->chip, LM3697_REG_OUTPUT_CFG, 0x04);
	lm3697_bl_write_byte(lm3697_bl->chip, LM3697_REG_BOOST, 0x06);
	lm3697_bl_write_byte(lm3697_bl->chip, LM3697_REG_CURR_SINK_EN, 0x03);
