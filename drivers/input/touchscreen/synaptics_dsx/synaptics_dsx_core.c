...
/*
*qijin add GESTURE_CONTROL for opening or closing gesture wakeup function
*/
...
#endif
#ifdef GESTURE_CONTROL
static struct proc_dir_entry *g_lct_wake_enable_proc = NULL;
#define LCT_WAKE_ENABLE_PROC_FILE "gesture_open"
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		retval = request_threaded_irq(rmi4_data->irq, NULL,
				synaptics_rmi4_irq, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
		...
		unsigned char ctrlreg, unsigned char subpacket)
{
	...
	query_6 = kzalloc(query_5->size_of_query6, GFP_KERNEL);
	if (!query_6) {
		dev_err(rmi4_data->pdev->dev.parent,
...
flash_prog_mode:
	...
	f01_query = kzalloc(F01_STD_QUERY_LEN, GFP_KERNEL);
	if (!f01_query) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static ssize_t lct_wake_enable_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	//char tmp_data[256] = {0};
	unsigned int input;
	unsigned int update_suspend=0;
	//struct device *dev;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(syna_dev);
...
if (rmi4_data->suspend){
		synaptics_rmi4_resume(syna_dev);
		update_suspend =1;
}
...
if (update_suspend==1){
		synaptics_rmi4_suspend(syna_dev);
}
...
static ssize_t lct_wake_enable_proc_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	int cnt= 0;
    char *page = NULL;
	//struct device *dev;
...
static const struct file_operations lct_wake_enable_proc_fops = {
	.read		= lct_wake_enable_proc_read,
	.write		= lct_wake_enable_proc_write,
};
...
#ifdef GESTURE_CONTROL
		g_lct_wake_enable_proc = proc_create_data(LCT_WAKE_ENABLE_PROC_FILE, 0777, NULL, &lct_wake_enable_proc_fops, NULL);
		if (IS_ERR_OR_NULL(g_lct_wake_enable_proc))
		{
			printk("create_proc_entry g_lct_wake_enable_proc failed\n");
		}
		else
		{
			printk("create_proc_entry g_lct_wake_enable_proc success\n");
		}
#endif	
...
/*
*func:no_sleep_in
*qijin add for reducing the Tp power consumption
*/
static void no_sleep_in(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
	int retval;
	unsigned char device_ctrl;
