...
static int hall_power_on(struct device *pdev)
{
	...
	if (IS_ERR(hall_vio)) {
		ret = -1;
		dev_err(pdev, "Regulator get failed vdd-io ret=%d\n", ret);
		return ret;
	}
	...
	if (regulator_count_voltages(hall_vio) > 0) {
		...
		if (ret) {
			dev_err(pdev, "Regulator set_vtg failed vdd-io ret=%d\n", ret);
			goto reg_vio_put;
		}
	}
	...
	if (ret) {
		dev_err(pdev, "Regulator vdd-io enable failed ret=%d\n", ret);
		return ret;
	}
		...
		struct class_attribute *attr, char *buf)
{
	...
	pr_err("hall_state_show state = %d, custome_state=%d\n", global_hall_info->hall_switch_state, state);
...
static int hall_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (err) {
			dev_err(&pdev->dev, "hall_probe DT parsing failed\n");
			goto free_struct;
		}
	} else{
		return -ENOMEM;
