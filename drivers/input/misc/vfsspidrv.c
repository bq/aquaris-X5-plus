...
static void vfsspi_test(struct spi_device *spi)
{
	...
	ret = spi_sync(spi, &m);
	if (ret)
		pr_err("getver command on EP1OUT failed %d!\n", ret);
	...
	ret = spi_sync(spi, &m);
	if (ret) {
		pr_err("spi read gerver failed !\n");
	} else {
		for (i = 0; i < t.len; i++)
			pr_debug("read gerver rx_buf[%d]: 0x%x\n",
				i, (u8)rx_buf[i]);
	}
	...
	struct vfsspi_device_data *data)
{
	...
	if (!gpio_is_valid(gpio)) {
		pr_err("get vdd33_en-gpio failed !\n");
		goto dt_exit;
	} else {
		data->vdd33_en = gpio;
	}

	pr_debug("drdy_pin=%d, sleep_pin=%d, vdd33_en=%d\n",
		data->drdy_pin,
		data->sleep_pin, data->vdd33_en);
...
static int vfsspi_power_init(struct vfsspi_device_data* data)
{
    int status = 0;
	struct regulator *vcc;

	vcc = regulator_get(&(data->spi->dev), "vcc");
	if (IS_ERR(vcc)) {
		pr_err("failed to get regulator vcc !\n");
		status = -EBUSY;
		goto vfsspi_power_get_failed;
	}
	status = regulator_enable(vcc);
	if (status) {
		pr_err("failed to enable regulator vcc !\n");
		regulator_put(vcc);
		status = -EBUSY;
		goto vfsspi_power_enable_failed;
	}

	status = gpio_direction_output(data->vdd33_en, 1);
	if (status < 0) {
...
vfsspi_power_enable_failed:
	regulator_put(vcc);
vfsspi_power_get_failed:
	return status;

}

static int vfsspi_init_gpio(struct vfsspi_device_data* data)
{
    int status = 0;
/*
	struct gpio all_gpio [] = {
		{data->drdy_pin , GPIOF_DIR_IN , "syna_irq_gpio"},
		{data->sleep_pin ,GPIOF_OUT_INIT_HIGH , "syna_reset_gpio"},
		{data->vdd18_en ,GPIOF_OUT_INIT_HIGH  , "syna_vdd18_en"},
		{data->vdd33_en , GPIOF_OUT_INIT_HIGH , "syna_vdd33_en"},
	}; 

	gpio_request_array(all_gpio , ARRAY_SIZE(all_gpio));
*/
    status = gpio_request(data->drdy_pin, "syna_irq_gpio");
	if (status < 0) {
		pr_err("gpio vfsspi_drdy request failed\n");
		return status;
	}	

    status = gpio_request(data->sleep_pin, "syna_reset_gpio");	
	if (status < 0) {
		pr_err("gpio vfsspi_sleep request failed\n");
        gpio_free(data->drdy_pin);
		return status;
	}

    status = gpio_request(data->vdd33_en, "syna_vdd33_en");
	if (status < 0) {
		pr_err("gpio vfsspi_vdd33_en request failed\n");
        gpio_free(data->drdy_pin);
        gpio_free(data->sleep_pin);		
		return status;
	}
...
#endif
{
...
#ifndef IN_QSEE
	pr_info("dev: %s, mode: 0x%x\n", dev_name(&spi->dev), spi->mode);
#else
	pr_info("dev: %s\n", dev_name(&spi->dev));
#endif
    ...
    if (vfsspi_device == NULL) {
        pr_err("alloc vfsspi_device_data failed !\n");
		return -ENOMEM;
    }
    ...
    wake_lock_init(&vfsspi_device->wake_lock,
		WAKE_LOCK_SUSPEND, "fingerprint_wakelock");
	...
	status = request_irq(gpio_irq, vfsspi_irq, IRQF_TRIGGER_RISING,
			"vfsspi_irq", vfsspi_device);
	if (status) {
	...
	pr_notice("fps driver probe successfully");
...
#ifndef IN_QSEE
	{ .compatible = "validity,metallica-spi",},
#else
...
#endif
	if (status < 0) {
		pr_err("install vfsspi driver failed !\n");
		return status;
	}
