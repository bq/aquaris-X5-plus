...
#ifdef pr_debug
#undef pr_debug
#define pr_debug(format, ...) printk(KERN_ERR "yht-tfa98xx, " format, ## __VA_ARGS__)
#endif
*/
		       ...
		       struct snd_ctl_elem_info *uinfo)
{
	...
	pr_debug("vsteps count: %d [prof=%d]\n", tfacont_get_max_vstep(0, profile),profile);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ready & tfa98xx_is_amp_running(0)) {
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	pr_debug("At %d In (%s),nprof=%d\n",__LINE__, __FUNCTION__,nprof);
	...
	pr_debug("At %d In (%s),mix_index=%d,name=%s,tfa98xx->fw.name=%s\n",__LINE__, __FUNCTION__,mix_index,tfa98xx_controls[mix_index].name,tfa98xx->fw.name);
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0 &&
			...
			pr_debug("At %d In (%s), profile added [%d]: %s\n",__LINE__, __FUNCTION__, bprofile->item_id, bprofile->basename);
			...
			if (tfacont_get_max_vstep(0, prof)) {
				...
				pr_debug("At %d In (%s),mix_index=%d,name=%s\n",__LINE__, __FUNCTION__,mix_index,tfa98xx_controls[mix_index].name);
				mix_index++;
			}
		}
	...
	pr_debug("At %d In (%s), will run snd_soc_add_codec_controls\n",__LINE__, __FUNCTION__);
...
static void tfa98xx_add_widgets(struct tfa98xx *tfa98xx)
{
	...
	pr_debug("At %d In (%s), add widgets\n",__LINE__, __FUNCTION__);
	...
	pr_debug("At %d In (%s), add widgets end\n",__LINE__, __FUNCTION__);
}
...
int tfa98xx_register_dsp(struct tfa98xx *tfa98xx)
{
	...
	for (i = 0; i < tfa98xx_cnt_max_device(); i++) {
		...
		pr_debug("%s: i=%d - dev = %02x addr = %02x\n", __func__, i, slave, tfa98xx->i2c->addr);
...
static char *tfa9891_fw_name = "tfa98xx.cnt";
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	pr_debug("At %d In (%s),tfa98xx->handle = %d\n",__LINE__, __FUNCTION__,tfa98xx->handle);
...
static int tfa98xx_load_container(struct tfa98xx *tfa98xx)
{
	...
	pr_debug("At %d In (%s), will run request_firmware_nowait, firmware=%s\n",__LINE__, __FUNCTION__,firmware);
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if ((!tfa98xx->input) && (tfa98xx_profile < 2)) {
		/*
	...
	struct snd_soc_dai *dai)
{
	...
	len = strlen(basename);
	pr_debug("At %d In (%s), nprof=%d,basename=%s,len=%d\n",__LINE__, __FUNCTION__,nprof,basename,len);
	...
	int clk_id, unsigned int freq, int dir)
{
	...
	tfa98xx->sysclk = freq;
	pr_debug("At %d In (%s),freq=%d,dir=%d\n",__LINE__, __FUNCTION__,freq,dir);
	return 0;
}
...
static int tfa98xx_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	pr_debug("At %d In (%s), fmt=0x%x,(fmt & SND_SOC_DAIFMT_FORMAT_MASK)=%d\n",__LINE__, __FUNCTION__,fmt,(fmt & SND_SOC_DAIFMT_FORMAT_MASK));
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	pr_debug("At %d In (%s),Requested rate: %d, sample size: %d\n",__LINE__, __FUNCTION__, rate,
	...
	pr_debug("At %d In (%s),mixer profile:container profile = [%d:%d]\n",__LINE__, __FUNCTION__, tfa98xx_mixer_profile, prof_idx);
...
static int _tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	struct snd_soc_codec *codec = dai->codec;
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
...
exit_release_lock:
	mutex_unlock(&tfa98xx->mute_lock);
	return 0;
}

#ifdef GOHAN_FM_POP_CC_GREEN
extern atomic_t fm_port_on;
static void tfa98xx_mute_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct tfa98xx *tfa98xx = container_of(dwork,
		struct tfa98xx, mute_dwork);
	struct snd_soc_codec *codec = tfa98xx->codec;
	struct snd_soc_card *card = codec->card;
	struct snd_soc_dai *_dai, *dai = NULL;

	list_for_each_entry(_dai, &card->dai_dev_list, card_list) {
		if (_dai->codec == codec) {
			dai = _dai;
			break;
		}
	...
	if (!dai) {
		pr_err("dai not found !\n");
		return;
	}

	pr_info("enter\n");
	_tfa98xx_mute(dai, 0, SNDRV_PCM_STREAM_PLAYBACK);
}
#endif
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (atomic_read(&fm_port_on) &&
		stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pr_info("mute = %d\n", mute);
		if (mute == 1) {
			cancel_delayed_work_sync(&tfa98xx->mute_dwork);
			return _tfa98xx_mute(dai, mute, stream);
		} else {
			schedule_delayed_work(&tfa98xx->mute_dwork,
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_debug("At %d In (%s), will run tfa98xx_load_container\n",__LINE__, __FUNCTION__);
...
#endif
	pr_debug("At %d In (%s), will run tfa98xx_add_widgets \n",__LINE__, __FUNCTION__);
...
/*	ret = regulator_enable(tfa98xx->vdd);
	if (ret != 0) {
		pr_debug("yht At %d In (%s), failed to enable tfa98xx->vdd,ret=%d\n",__LINE__, __FUNCTION__,ret);
		return 4;
	}*/
	return 0;
}
...
/*	pr_debug("At %d In (%s),dont have power config, VREG_L6_1P8\n",__LINE__, __FUNCTION__);
	tfa98xx->vdd = devm_regulator_get(dev, "Vdd");
	if (IS_ERR(tfa98xx->vdd)) {
		ret = PTR_ERR(tfa98xx->vdd);
		pr_debug("At %d In (%s),devm_regulator_get error ret=%d\n",__LINE__, __FUNCTION__,ret);
		return 4;
	}*/
	...
	const struct i2c_device_id *id)
{
	...
	mutex_init(&tfa98xx->mute_lock);
#ifdef GOHAN_FM_POP_CC_GREEN
	INIT_DELAYED_WORK(&tfa98xx->mute_dwork, tfa98xx_mute_work);
#endif
	...
	if (np) {
		...
		pr_debug("At %d In (%s),reset_gpio=%d,irq_gpio=%d\n",__LINE__, __FUNCTION__,tfa98xx->reset_gpio,tfa98xx->irq_gpio);
		if (no_start)
			tfa98xx->irq_gpio = -1;
	} else {
	...
	pr_debug("At %d In (%s),will run tfa98xx_ext_reset\n",__LINE__, __FUNCTION__);
...
// add this for test only
	...
	ret = regmap_read(tfa98xx->regmap, 0x03, &reg);
	if (ret < 0) {
		;//pr_debug("At %d In (%s),ret=%d,i=%d\n",__LINE__, __FUNCTION__,ret,i++);
	} else {
		; //pr_debug("At %d In (%s),ret=%d,i=%d\n",__LINE__, __FUNCTION__,ret,i);
		goto test_next;
	}
...
if (tfa98xx && gpio_is_valid(tfa98xx->reset_gpio)) {	
		...
		gpio_set_value_cansleep(tfa98xx->reset_gpio, 0);
		mdelay(2);
		pr_debug("At %d In (%s),in reset test\n",__LINE__, __FUNCTION__);
}	
#endif

	tfa98xx->rev = reg & 0xFF;
	pr_debug("At %d In (%s),ret=%d,tfa98xx->rev=0x%x\n",__LINE__, __FUNCTION__,ret,tfa98xx->rev);
	...
	pr_debug("At %d In (%s), ret=%d,will run snd_soc_register_codec\n",__LINE__, __FUNCTION__,ret);
	...
	pr_debug("At %d In (%s), ret=%d,will request irq thread\n",__LINE__, __FUNCTION__,ret);
	...
	if (gpio_is_valid(tfa98xx->irq_gpio) &&
		...
		pr_debug("At %d In (%s),will run tfa98xx_interrupt_setup \n",__LINE__, __FUNCTION__);
	...
	if (CONFIG_DEBUG_FS) pr_debug("At %d In (%s),CONFIG_DEBUG_FS is defined \n",__LINE__, __FUNCTION__);
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	cancel_delayed_work_sync(&tfa98xx->mute_dwork);
#endif
	mutex_destroy(&tfa98xx->mute_lock);
...
/*	ret = regulator_disable(tfa98xx->vdd);
	if (ret != 0) {
		pr_debug("yht At %d In (%s), failed to diable tfa98xx->vdd,ret=%d\n",__LINE__, __FUNCTION__,ret);
		return 4;
	}*/
